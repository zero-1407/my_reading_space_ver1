...
const SHOPS = {
  post: {
    ...
    action: () => openPost(),
    decor(t) {
      for (let r = 0; r < 3; r++) for (let c = 0; c < 7; c++) {    // 번호 매겨진 우편함 칸 — 3줄 7칸, 더 빽빽하게
        const bx = 14 + c * 15, by = 12 + r * 16;
        px(bx, by, 13, 14, (r + c) % 2 ? '#E8C46A' : '#F0DFA0');
        px(bx + 9, by + 6, 2, 2, '#8A6444');                        // 놋쇠 손잡이
      }
      px(12, 10, 106, 2, '#8A6444');
      shopWindow(230, 12, 40, 34);
      // 소포 더미 — 크기·색 다양하게, 끈으로 묶은 것도 섞어서
      [{ x:18, y:100, w:18, h:14, c:'#C4A876' }, { x:38, y:106, w:14, h:8, c:'#B4986A' },
       { x:16, y:112, w:12, h:8,  c:'#D9C098' }].forEach(p => {
        px(p.x - 1, p.y + p.h, p.w + 2, 2, 'rgba(60,42,24,.15)');
        px(p.x, p.y, p.w, p.h, p.c);
        px(p.x, p.y + p.h / 2 - 1, p.w, 2, shade(p.c, .72));
        px(p.x + p.w / 2 - 1, p.y, 2, p.h, shade(p.c, .72));
      });
      px(58, 98, 3, 4, '#6E5238'); px(56, 90, 7, 8, '#B0A88E');            // 소포 저울
      px(58, 84, 1, 6, '#6E5238'); px(54, 82, 9, 3, '#8A6444');
      blob(190, 22, 8, '#8A6444'); blob(190, 22, 6.5, '#FBF3E2');          // 벽시계
      px(189, 18, 1, 5, '#3A2E28'); px(190, 22, 4, 1, '#3A2E28');
  ...
  cafe: {
    ...
    decor(t) {
      shopWindow(230, 12, 40, 34);
      px(90, 4, 2, 14, '#5A7A52'); blob(90, 20, 7, '#6E9A6E'); blob(87, 16, 3, '#7CBC72'); // 매달린 화분
      [[16, 78], [58, 84]].forEach(([x, y]) => {                          // 원탁 두 개 + 의자
        px(x - 1, y + 20, 26, 2, 'rgba(60,42,24,.15)');
        px(x, y, 24, 20, '#6E4E3A'); px(x, y, 24, 3, '#8A6A50');
        blob(x + 6, y - 4, 3, '#EFE4D0'); blob(x + 6, y - 4, 1.6, '#FBF3E2');  // 찻잔
        blob(x + 16, y - 4, 3, '#D4645C');                                    // 디저트 접시
        px(x + 27, y + 4, 8, 12, '#5A4030');
      });
      // 유리 진열장 — 디저트 세 종류 (계산대 점원 자리는 피해서)
      const px0 = 100, py0 = 58, pw = 58, ph = 26;
      px(px0 - 2, py0 - 2, pw + 4, ph + 4, '#8A5A6E');
      px(px0, py0, pw, ph, 'rgba(230,222,232,.55)');
      px(px0 + 2, py0 + ph * .5, pw - 4, 1, 'rgba(255,255,255,.35)');
      [[px0 + 12, '#D8B888'], [px0 + 32, '#C48A6E'], [px0 + 50, '#E8C46A']].forEach(([x, c]) => {
